...
test_curl_check_reply()
{
    ...
    curl --retry 1 --connect-timeout 1 --max-time 10 "$host":"$port" | grep "$message" > /dev/null 2>&1
    if test $? -ne 0; then
...
test_netcat_connect()
{
    ...
    # Check connection.
    netcat -w 3 "$host" "$port" < /dev/null > /dev/null 2>&1
    if test $? -ne 0; then
        echo "Cannot connect to $host:$port" 1>&2
        echo "FAILED"
        clean_up
...
test_redis_connect()
{
    ...
    redis-cli -h "$host" -p "$port" < /dev/null > /dev/null 2>&1
    if test $? -ne 0; then
...
test_redis_cli()
{
    ...
    redis-cli -h "$host" -p "$port" set a 1 > /dev/null 2>&1
    redis-cli -h "$host" -p "$port" get a > /dev/null 2>&1
    if test $? -eq 1; then
        echo "FAILED"
